...
class Total:
    ...
    def lane_callback(self, msg: LaneInformation):
        if self.rotary_exit_flag: return
    ...
    def run(self):
        ...
        if self.dist(self.goal_list[1][self.sequence].target_pose.pose.position) < self.lookahead_distance:
            if self.sequence == len(self.goal_list[1]) - 1:
                print(self.min_dis.dis)
                if self.min_dis.dis < 1.7:
                    #print("1.7")
                    self.stop_flag = True
                    self.stop()
        ...
        dy = self.goal_list[1][self.sequence].target_pose.pose.position.y - self.now_pose.y
        dx = self.goal_list[1][self.sequence].target_pose.pose.position.x - self.now_pose.x
